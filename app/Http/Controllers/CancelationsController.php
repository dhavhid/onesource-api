...
class CancelationsController extends Controller {
    ...
    {
        $reasons = DB::table('reasons')->where('is_confirmation',0)->whereNull('deleted_at')->orderBy('name','asc')->select('id','name')->paginate(Config::get('constants.per_page'));
    ...
    public function search(Request $request)
    {
        ...
        if (strlen($var) > 0) {
            $results = DB::table('reasons')->where('name','ilike',$GLOBALS['q'])->where('is_confirmation','=',0)->whereNull('deleted_at')->select($fields)->paginate(10);
